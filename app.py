...
# --- دالة الاتصال ---
def get_connection():
    # استخدام st.connection للحصول على اتصال PostgreSQL من secrets
...
def add_driver(driver_id, name, bike_plate, whatsapp, notes, is_active):
    ...
    try:
        with conn.session as s:
            ...
            s.commit()
        # مسح الكاش للدوال التي تعتمد على البيانات
        try:
...
# --- دوال جلب البيانات (آمنة للكاش أو بدون كاش حسب نوع النتيجة) ---
@st.cache_data(ttl=60)
def search_driver(search_term):
    # ترجع dict بسيط أو None => آمنة للكاش لأن الاستدعاء يعتمد على str
    conn = get_connection()
...
def get_driver_info(driver_id):
    conn = get_connection()
    df = conn.query(text("SELECT name, COALESCE(balance,0) as balance, is_active FROM drivers WHERE driver_id=:id"), params={"id": driver_id})
    if not df.empty:
        result = df.iloc[0]
        return {"name": result['name'], "balance": float(result['balance']), "is_active": bool(result['is_active'])}
    return None

@st.cache_data(ttl=60)
def get_deliveries_count_per_driver():
    # نعيد قائمة من dicts (قابلة للتخزين في الكاش) بدل DataFrame
    conn = get_connection()
    query = text("""
    SELECT
    ...
    if df.empty:
        return []
    # التأكد من تحويل القيم إلى native types
    records = []
    for _, row in df.iterrows():
        records.append({"driver_id": str(row['driver_id']), "عدد التوصيلات": int(row['deliveries_count'])})
    return records

def get_all_drivers_details():
    # هذه الدالة لا تستخدم الكاش لأنها تبني وتدمج DataFrames
    conn = get_connection()
    query_drivers = text("SELECT driver_id, name as \"الاسم\", bike_plate as \"رقم اللوحة\", whatsapp as \"واتساب\", COALESCE(balance,0) as \"الرصيد\", is_active as \"الحالة\", notes as \"ملاحظات\" FROM drivers")
    df = conn.query(query_drivers)
    if df.empty:
        # إرجاع DataFrame فارغ بصيغة الأعمدة المتوقعة
        empty = pd.DataFrame(columns=['ت','الترقيم','الاسم','رقم اللوحة','واتساب','الرصيد','عدد التوصيلات','الحالة','ملاحظات'])
        return empty

    # جلب عدد التوصيلات (من دالة مخبأة ترجع list of dicts)
...
def get_totals():
    ...
    total_deducted_raw = conn.query("SELECT COALESCE(SUM(amount),0) FROM transactions WHERE type='خصم توصيلة'").iloc[0,0]
    total_deducted = abs(float(total_deducted_raw))
...
# هذه الدالة قد تُرجع DataFrame لذا لا نضع عليها كاش
def get_history(driver_id=None):
    ...
    if driver_id:
        ...
        df = conn.query(text("SELECT driver_name as \"المندوب\", type as \"العملية\", amount as \"المبلغ\", timestamp as \"التوقيت\" FROM transactions ORDER BY id DESC"))
    return df

# --- التهيئة والواجهة (الجزء الكامل كما في الكود الأصلي) ---
    ...
    if report_type == "التقارير الإجمالية":
        ...
        if selected_id:
            ...
            if driver_name_data:
                ...
                st.error("حدث خطأ في جلب بيانات المندوب.")
        else:
            st.info("يرجى استخدام شريط البحث أعلاه لتحديد المندوب المطلوب.")

# ------------------ إعدادات التطبيق (الشعار) ------------------
